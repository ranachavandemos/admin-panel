import { ApprovalRequest, MISTeacher, MISStudent, AuditTrail, } from "../models/index.js";
import { parseCSVBuffer, parseXLSXBuffer, validateStudentRow, validateTeacherRow, } from "../utils/csv.js";
export default async function uploadRoutes(fastify) {
    fastify.post("/upload/direct", { preHandler: [fastify.optionalAuth] }, async (req, reply) => {
        try {
            ...
            const validRows = cleanData.filter((row) => {
                const validation = entityType === "student"
            ...
            if (!validRows.length) {
                return reply.code(400).send({
                    error: "No valid records found. Please check required fields and remove empty rows.",
                });
            }
            fastify.log.info({
            ...
            if (role === "admin") {
                ...
                if (entityType === "student") {
                    ...
                    if (newRecords.length)
                        await MISStudent.bulkCreate(newRecords);
                }
                else {
                    const ids = validRows
                    ...
                    if (newRecords.length)
                        await MISTeacher.bulkCreate(newRecords);
                }
